            ...
            if (page.items.length > 0) {
                ...
                textItemsGroupedByLine.forEach(lineTextItems => {
                    ...
                    if (lineItem.parsedElements.footnoteLinks.length > 0) {
                        const footnoteLinks = lineItem.parsedElements.footnoteLinks.map((footnoteLink:any) => `<span key={ footnoteLink }><a href={ "#Page " + (page.index + 1) }>{ footnoteLink }</a>,</span>`);
                    ...
                    if (lineItem.parsedElements.footnotes.length > 0) {
                        ...
                        const footnotes = lineItem.parsedElements.footnotes.map((footnote:any) => `<span key={ footnote }><a href={ "#Page " + (page.index + 1) }>{ footnote }</a>,</span>`);
