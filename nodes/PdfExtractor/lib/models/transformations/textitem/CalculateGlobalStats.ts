...
export default class CalculateGlobalStats extends ToTextItemTransformation {
	...
	transform(parseResult: any) {
		...
		const distanceToOccurrence: any = {};
		parseResult.pages.forEach((page: any) => {
		...
		const fontIdToName: any[] = [];
		const fontToFormats = new Map();
		this.fontMap.forEach(function (value: any, key: any) {
		...
		const newPages = parseResult.pages.map((page: any) => {
			return {
				...page,
